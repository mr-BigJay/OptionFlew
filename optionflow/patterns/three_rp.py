...
# LuxAlgo brpType — پیش‌فرض Enhanced (Normal عملکرد ضعیف‌تر دارد)
...
def _matches_pattern_type(
    *,
    ...
    """همان فیلتر brpType در Pine (All / Enhanced / Normal)."""
...
def detect_three_rp(
    ...
    *,
    ...
    """LuxAlgo Three Bar Reversal — فقط 1h · brpType پیش‌فرض Enhanced."""
    if timeframe != "1h":
        return None
    if len(bars) < 3:
        return None
    ...
    for off in range(max_back + 1):
        ...
        if _is_bullish_reversal(bars, i):
            ...
            return PatternHit(
                ...
                status_fa=status,
                summary_fa=(
                    f"برگشت سه‌کندلی صعودی (enhanced)؛ "
                    f"بسته {b2.close:,.0f} > سقف کندل۱ {pattern_high:,.0f} · "
                    f"کف ساختار {support:,.0f}."
                ),
                forecast_fa=(
                    "پیش‌بینی: الگوی Enhanced تأییدشده روی close بالای high[2]؛ "
                    "شکست زیر کف الگو = باطل."
                ),
                meta={
                    "direction": "up",
                    "stage": stage,
        ...
        if _is_bearish_reversal(bars, i):
            ...
            return PatternHit(
                ...
                status_fa=status,
                summary_fa=(
                    f"برگشت سه‌کندلی نزولی (enhanced)؛ "
                    f"بسته {b2.close:,.0f} < کف کندل۱ {pattern_low:,.0f} · "
                    f"سقف ساختار {resistance:,.0f}."
                ),
                forecast_fa=(
                    "پیش‌بینی: الگوی Enhanced تأییدشده روی close زیر low[2]؛ "
                    "شکست بالای سقف الگو = باطل."
                ),
                meta={
                    "direction": "down",
                    "stage": stage,
